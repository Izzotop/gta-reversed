...
uint32_t& CStreaming::ms_nTimePassedSinceLastCopBikeStreamedIn = *reinterpret_cast<uint32_t*>(0x9654C0);
...
int32_t* CStreaming::ms_NextPedToLoadFromGroup = reinterpret_cast<int32_t*>(0x8E4BB8);
...
uint8_t** CStreaming::ms_pStreamingBuffer = reinterpret_cast<uint8_t**>(0x8E4CAC);
...
static int32_t& CurrentGangMemberToLoad = *(int32_t*)0x9654D4;
...
void CStreaming::InjectHooks()
{
    ...
    ReversibleHooks::Install("CStreaming", "LoadCdDirectory_char", 0x5B6170, (void(*)(const char*, int32_t)) & CStreaming::LoadCdDirectory);
    ...
    ReversibleHooks::Install("CStreaming", "ProcessEntitiesInSectorList", 0x40C270, (void(*)(CPtrList&, float, float, float, float, float, float, float, int32_t)) & CStreaming::ProcessEntitiesInSectorList);
...
void CStreaming::AddLodsToRequestList(CVector const& point, unsigned int streamingFlags) {
    ...
    int32_t startSectorX = std::max(CWorld::GetLodSectorX(minX), 0);
    int32_t startSectorY = std::max(CWorld::GetLodSectorY(minY), 0);
    int32_t endSectorX = std::min(CWorld::GetLodSectorX(maxX), MAX_LOD_PTR_LISTS_X - 1);
    int32_t endSectorY = std::min(CWorld::GetLodSectorY(maxY), MAX_LOD_PTR_LISTS_Y - 1);
...
void CStreaming::AddModelsToRequestList(CVector const& point, unsigned int streamingFlags) {
    ...
    const int32_t radius = static_cast<int32_t>(CWorld::GetHalfMapSectorX(fRadius));
    const int32_t squaredRadius1 = (radius - 1) * (radius - 1);
    const int32_t squaredRadius2 = (radius + 2) * (radius + 2);
    const int32_t pointX = CWorld::GetSectorX(point.x);
    const int32_t pointY = CWorld::GetSectorY(point.y);
    ...
    for (int32_t sectorY = startSectorY; sectorY <= endSectorY; ++sectorY) {
        const int32_t distanceY = sectorY - pointY;
        const int32_t squaredDistanceY = distanceY * distanceY;
        for (int32_t sectorX = startSectorX; sectorX <= endSectorX; ++sectorX) {
            CRepeatSector* pRepeatSector = GetRepeatSector(sectorX, sectorY);
            CSector* pSector = GetSector(sectorX, sectorY);
            const int32_t distanceX = sectorX - pointX;
            const int32_t dotProduct = distanceX * distanceX + squaredDistanceY;
...
void CStreaming::ClearFlagForAll(unsigned int streamingFlag) {
    for (int32_t i = 0; i < RESOURCE_ID_TOTAL; i++) {
...
void CStreaming::ClearSlots(int totalSlots) {
    for (int32_t i = 0; i < totalSlots; ++i) {
        int32_t modelId = ms_pedsLoaded[i];
        if (modelId >= 0) {
...
// 0x407F00
bool CStreaming::HasSpecialCharLoaded(int32_t slot)
{
    return ms_aInfoForModel[slot + SPECIAL_MODELS_RESOURCE_ID].m_nLoadState == LOADSTATE_LOADED;
}

// 0x407820
bool CStreaming::HasVehicleUpgradeLoaded(int32_t modelId)
{
    if (ms_aInfoForModel[modelId].m_nLoadState != LOADSTATE_LOADED)
        return false;
    int16_t otherUpgradeModelId = CVehicleModelInfo::ms_linkedUpgrades.FindOtherUpgrade(modelId);
...
void CStreaming::DeleteAllRwObjects() {
    for (int32_t sectorX = 0; sectorX < MAX_SECTORS_X; ++sectorX) {
        for (int32_t sectorY = 0; sectorY < MAX_SECTORS_Y; ++sectorY) {
            CRepeatSector* pRepeatSector = GetRepeatSector(sectorX, sectorY);
            CSector* pSector = GetSector(sectorX, sectorY);
...
bool CStreaming::DeleteLeastUsedEntityRwObject(bool bNotOnScreen, unsigned int streamingFlags) {
    ...
    while (previousLink != &ms_rwObjectInstances.usedListHead) {
        ...
        if (!pEntity->m_bImBeingRendered && !pEntity->m_bStreamingDontDelete) {
            const int32_t modelId = pEntity->m_nModelIndex;
...
void CStreaming::DeleteRwObjectsAfterDeath(CVector const& point) {
    int32_t pointX = CWorld::GetSectorX(point.x);
    int32_t pointY = CWorld::GetSectorY(point.y);
    for (int32_t sectorX = 0; sectorX < MAX_SECTORS_X; ++sectorX) {
...
// 0x40D7C0
void CStreaming::DeleteRwObjectsBehindCamera(int32_t memoryToCleanInBytes) {
    if (static_cast<int32_t>(CStreaming::ms_memoryUsed) < memoryToCleanInBytes)
        return;

    const CVector& cameraPos = TheCamera.GetPosition();
    int32_t pointX = CWorld::GetSectorX(cameraPos.x);
    int32_t pointY = CWorld::GetSectorY(cameraPos.y);
    const CVector2D& cameraForward = TheCamera.GetForward();
    if (fabs(cameraForward.y) < fabs(cameraForward.x)) {
        int32_t sectorStartY = std::max(pointY - 10, 0);
        int32_t sectorEndY = std::min(pointY + 10, MAX_SECTORS_Y - 1);
        int32_t sectorStartX = 0;
        int32_t sectorEndX = 0;
        int32_t factorX = 0;
        ...
        CWorld::IncrementCurrentScanCode();
        for (int32_t sectorX = sectorStartX; sectorX != sectorEndX; sectorX += factorX) {
            for (int32_t sectorY = sectorStartY; sectorY <= sectorEndY; sectorY++) {
                CRepeatSector* pRepeatSector = GetRepeatSector(sectorX, sectorY);
                CSector* pSector = GetSector(sectorX, sectorY);
        ...
        int32_t sectorStartX = std::max(pointX - 10, 0);
        int32_t sectorEndX = std::min(pointX + 10, MAX_SECTORS_X - 1);
        int32_t sectorStartY = 0;
        int32_t sectorEndY = 0;
        int32_t factorY = 0;
        ...
        CWorld::IncrementCurrentScanCode();
        for (int32_t sectorY = sectorStartY; sectorY != sectorEndY; sectorY += factorY) {
            for (int32_t sectorX = sectorStartX; sectorX <= sectorEndX; sectorX++) {
                CRepeatSector* pRepeatSector = GetRepeatSector(sectorX, sectorY);
                CSector* pSector = GetSector(sectorX, sectorY);
                if (DeleteRwObjectsBehindCameraInSectorList(pSector->m_buildings, memoryToCleanInBytes) ||
                    DeleteRwObjectsBehindCameraInSectorList(pSector->m_dummies, memoryToCleanInBytes) ||
    ...
    while ((static_cast<int32_t>(ms_memoryUsed) >= memoryToCleanInBytes)) {
...
// 0x409940
bool CStreaming::DeleteRwObjectsBehindCameraInSectorList(CPtrList& list, int32_t memoryToCleanInBytes) {
    for (CPtrNode* pNode = list.GetNode(); pNode; pNode = pNode->pNext) {
        ...
        if (pEntity->m_nScanCode != CWorld::ms_nCurrentScanCode) {
                ...
                && FindPlayerPed(-1)->m_pContactEntity != pEntity)
            {
                ...
                if (!CModelInfo::ms_modelInfoPtrs[modelId]->m_nRefCount) {
                    RemoveModel(modelId);
                    if (static_cast<int32_t>(ms_memoryUsed) < memoryToCleanInBytes)
                        return true;
                }
            }
        }
...
// 0x407A70
void CStreaming::DeleteRwObjectsInSectorList(CPtrList& list, int32_t sectorX, int32_t sectorY) {
    for (CPtrNode* pNode = list.GetNode(); pNode; pNode = pNode->pNext) {
        CEntity* pEntity = reinterpret_cast<CEntity*>(pNode->pItem);
...
// 0x4099E0
bool CStreaming::DeleteRwObjectsNotInFrustumInSectorList(CPtrList& list, int32_t memoryToCleanInBytes) {
    for (CPtrNode* pNode = list.GetNode(); pNode; pNode = pNode->pNext) {
        CEntity* pEntity = reinterpret_cast<CEntity*>(pNode->pItem);
        if (pEntity->m_nScanCode != CWorld::ms_nCurrentScanCode) {
            pEntity->m_nScanCode = CWorld::ms_nCurrentScanCode;
...
// 0x40D2F0
bool CStreaming::RemoveReferencedTxds(int32_t memoryToCleanInBytes)
{
    ...
    while (streamingInfo != CStreaming::ms_startLoadedList) {
        ...
        const int32_t modelId = streamingInfo - CStreaming::ms_aInfoForModel;
...
// 0x5B6170
void CStreaming::LoadCdDirectory(const char* filename, int32_t archiveId)
{
    FILE* file = CFileMgr::OpenFile(filename, "rb");
    if (!file)
        return;

    int32_t previousModelId = -1;
    char version[4];
    int32_t entryCount;
    ...
    for (int32_t i = 0; i < entryCount; i++) {
        ...
        const int32_t nameSize = sizeof(CDirectory::DirectoryInfo::m_szName);
        ...
        *pExtension = 0;
        ...
        if (!_memicmp(pExtension + 1, "DFF", 3u)) {
            ...
            int32_t txdSlot = CTxdStore::FindTxdSlot(entryInfo.m_szName);
            if (txdSlot == -1) {
...
void CStreaming::LoadCdDirectory()
{
    ...
    ms_imageOffsets[5] = -1;
    ////////////////////////////////////
    for (int32_t archiveId = 0; archiveId < TOTAL_IMG_ARCHIVES; archiveId++) {
...
void CStreaming::LoadZoneVehicle(const CVector& point) {
    if (CPopCycle::m_pCurrZoneInfo) {
        ...
        int32_t carGroupId = CPopCycle::PickARandomGroupOfOtherPeds();
        int32_t modelId = CCarCtrl::ChooseCarModelToLoad(carGroupId);
        if (modelId >= 0)
...
void CStreaming::RequestBigBuildings(CVector const& point) {
    for (int32_t i = CPools::ms_pBuildingPool->GetSize() - 1; i >= 0; i--) {
...
// 0x408C70
void CStreaming::RequestVehicleUpgrade(int32_t modelId, uint32_t streamingFlags)
{
    RequestModel(modelId, streamingFlags);
    int16_t otherUpgradeModelId = CVehicleModelInfo::ms_linkedUpgrades.FindOtherUpgrade(modelId);
    if (otherUpgradeModelId != -1)
...
// 0x407FB0
int32_t CStreaming::FindMIPedSlotForInterior(int32_t randFactor)
{
    for (int32_t i = 0; i < TOTAL_LOADED_PEDS; i++) {
        int32_t modelId = ms_pedsLoaded[(i + randFactor) % TOTAL_LOADED_PEDS];
...
void CStreaming::RequestModelStream(int channelId)
{
    ...
    for (int32_t i = modelIndex; i < numberOfModelIds; i++) {
...
void CStreaming::ReInit() {
    ...
    for (int32_t modelId = 0; modelId < RESOURCE_ID_TOTAL; modelId++)
    {
        CStreamingInfo& streamingInfo = ms_aInfoForModel[modelId];
    ...
    for (int32_t scmId = 0; scmId < TOTAL_SCM_MODEL_IDS; scmId++) {
    ...
    for (int32_t i = 0; i < TOTAL_SPECIAL_MODELS; i++) {
        const int32_t modelId = i + SPECIAL_MODELS_RESOURCE_ID;
        RemoveModel(modelId);
        CModelInfo::ms_modelInfoPtrs[modelId]->SetModelName(gta_empty_string);
    }
    for (int32_t i = 0; i < TOTAL_CLOTHES_MODELS; i++) {
        const int32_t modelId = i + CLOTHES_MODELS_RESOURCE_ID;
        RemoveModel(modelId);
        CModelInfo::ms_modelInfoPtrs[modelId]->SetModelName(gta_empty_string);
    }
    for (int32_t i = 0; i < TOTAL_CUTSCENE_MODELS; i++) {
        const int32_t modelId = i + CUTSCENE_MODELS_RESOURCE_ID;
        RemoveModel(modelId);
        CModelInfo::ms_modelInfoPtrs[modelId]->SetModelName(gta_empty_string);
    }
}
...
void CStreaming::RemoveBuildingsNotInArea(int areaCode) {
    ...
    for (int32_t i = CPools::ms_pDummyPool->GetSize() - 1; i >= 0; i--) {
...
void CStreaming::RemoveCurrentZonesModels() {
    ...
    ms_numPedsLoaded = 0;
    RequestModel(MODEL_MALE01, STREAMING_GAME_REQUIRED);
    for (int32_t groupId = 0; groupId < TOTAL_GANGS; groupId++) {
        ...
        for (int32_t i = 0; i < 5; i++) {
            int32_t modelId = CPopulation::GetPedGroupModelId(pedGroupId, i);
...
bool CStreaming::RemoveLeastUsedModel(unsigned int streamingFlags) {
    ...
    for (; streamingInfo != ms_startLoadedList; streamingInfo = streamingInfo->GetPrev()) {
        const int32_t modelId = streamingInfo - ms_aInfoForModel;
        if (!(streamingFlags & streamingInfo->m_nFlags)) {
            if (modelId >= RESOURCE_ID_DFF && modelId < RESOURCE_ID_TXD) {
                ...
                const int32_t txdId = modelId - RESOURCE_ID_TXD;
                ...
                const int32_t animBlockId = modelId - RESOURCE_ID_IFP;
                ...
                const int32_t scmId = modelId - RESOURCE_ID_SCM;
...
bool CStreaming::CarIsCandidateForRemoval(int32_t modelId) {
    CStreamingInfo& streamingInfo = ms_aInfoForModel[modelId];
    if (!CModelInfo::ms_modelInfoPtrs[modelId]->m_nRefCount
        && !(streamingInfo.m_nFlags & (STREAMING_MISSION_REQUIRED | STREAMING_GAME_REQUIRED))
        && streamingInfo.m_nLoadState == LOADSTATE_LOADED)
...
bool CStreaming::RemoveLoadedVehicle() {
    for (int32_t i = 0; i < CPopulation::m_InAppropriateLoadedCars.CountMembers(); i++) {
        int32_t modelId = CPopulation::m_InAppropriateLoadedCars.GetMember(i);
        if (CarIsCandidateForRemoval(modelId)) {
            RemoveModel(modelId);
            return true;
        }
    }
    int32_t numBoatsNeeded = 0;
    if (CStreaming::m_bBoatsNeeded)
        numBoatsNeeded = 2;
    if (numBoatsNeeded < CPopulation::m_LoadedBoats.CountMembers()) {
        for (int32_t i = 0; i < CPopulation::m_LoadedBoats.CountMembers(); i++) {
            int32_t modelId = CPopulation::m_LoadedBoats.GetMember(i);
            if (CarIsCandidateForRemoval(modelId)) {
                RemoveModel(modelId);
                return true;
            }
    ...
    for (int32_t i = 0; i < appropriateLoadedCars.CountMembers(); i++) {
        int32_t modelId = appropriateLoadedCars.GetMember(i);
        if (CarIsCandidateForRemoval(modelId)) {
            RemoveModel(modelId);
            return true;
        }
...
bool CStreaming::ZoneModelIsCandidateForRemoval(int32_t modelId) {
    CStreamingInfo& streamingInfo = ms_aInfoForModel[modelId];
    if (!CModelInfo::ms_modelInfoPtrs[modelId]->m_nRefCount
        && !(streamingInfo.m_nFlags & (STREAMING_MISSION_REQUIRED | STREAMING_GAME_REQUIRED))
        && streamingInfo.m_nLoadState == LOADSTATE_LOADED)
...
// 0x40C270
void CStreaming::ProcessEntitiesInSectorList(CPtrList& list, float posX, float posY, float minX, float minY, float maxX, float maxY, float radius, int32_t streamingflags)
{
    ...
    for (CPtrNode* pNode = list.GetNode(); pNode; pNode = pNode->pNext) {
        ...
        if (pEntity->m_nScanCode != CWorld::ms_nCurrentScanCode) {
            pEntity->m_nScanCode = CWorld::ms_nCurrentScanCode;
            uint16_t modelId = pEntity->m_nModelIndex;
...
// 0x40C450
void CStreaming::ProcessEntitiesInSectorList(CPtrList& list, int32_t streamingFlags)
{
    for (CPtrNode* pNode = list.GetNode(); pNode; pNode = pNode->pNext) {
        CEntity* pEntity = reinterpret_cast<CEntity*>(pNode->pItem);
        if (pEntity->m_nScanCode != CWorld::ms_nCurrentScanCode) {
...
void CStreaming::RetryLoadFile(int channelId) {
    ...
    while (true) {
        ...
        if (bStreamRead) {
            uint8_t* pBuffer = CStreaming::ms_pStreamingBuffer[channelId];
...
{
    static int32_t currentChannel = 0;
    if (ms_bLoadingBigModel)
        currentChannel = 0;
    ...
    if (!ms_bLoadingBigModel) {
        const int32_t otherChannelId = 1 - currentChannel;
...
// 0x408000
bool CStreaming::AddToLoadedVehiclesList(int32_t modelId)
{
    ...
    const int32_t numBoats = CPopulation::m_nNumCarsInGroup[POPCYCLE_CARGROUP_BOATS];
    for (int32_t i = 0; i < numBoats; i++) {
    ...
    for (int32_t gangId = 0; gangId < TOTAL_GANGS; gangId++) {
        const int32_t groupId = gangId + POPCYCLE_CARGROUP_BALLAS;
        const uint16_t numCarsInGroup = CPopulation::m_nNumCarsInGroup[groupId];
        for (int32_t i = 0; i < numCarsInGroup; i++) {
            const int32_t vehicleModelId = CPopulation::m_CarGroups[groupId][i];
...
// 0x407D50
int32_t CStreaming::GetDefaultCabDriverModel()
{
    static int32_t randomIndex = 0;
    const int32_t defaultCabDriverModels[7] = { MODEL_BMOCD, MODEL_WMYCD1, MODEL_SBMOCD,
        MODEL_SWMOCD, MODEL_VBMOCD , MODEL_VWMYCD, -1 };
    const int32_t modelId = defaultCabDriverModels[randomIndex];
...
// 0x407C00
int32_t CStreaming::GetDefaultCopModel()
{
    const int32_t modelId = ms_aDefaultCopModel[CTheZones::m_CurrLevel];
    if (ms_aInfoForModel[modelId].m_nLoadState == LOADSTATE_LOADED)
        return modelId;
    for (int32_t i = 0; i < 4; i++) {
        const int32_t copModelId = ms_aDefaultCopModel[i];
...
void CStreaming::Init2()
{
    ...
    for (int32_t i = 0; i < TOTAL_DFF_MODEL_IDS; i++) {
        const int32_t modelId = i + RESOURCE_ID_DFF;
    ...
    for (int32_t i = 0; i < TOTAL_TXD_MODEL_IDS; i++) {
        const int32_t modelId = i + RESOURCE_ID_TXD;
        CStreamingInfo& streamingInfo = ms_aInfoForModel[modelId];
    ...
    const uint32_t maximumModelSizeInBytes = ms_streamingBufferSize * STREAMING_SECTOR_SIZE;
...
void CStreaming::InitImageList() {
    for (int32_t i = 0; i < TOTAL_IMG_ARCHIVES; i++) {
...
bool CStreaming::IsCarModelNeededInCurrentZone(int modelId) {
    ...
    for (int32_t groupId = 0; groupId < POPCYCLE_TOTAL_GROUP_PERCS; groupId++) {
...
void CStreaming::SetMissionDoesntRequireModel(int modelId) {
    for (int32_t i = modelId; ; i = CModelInfo::ms_modelInfoPtrs[i]->m_nTxdIndex + RESOURCE_ID_TXD) {
...
bool CStreaming::StreamAmbulanceAndMedic(bool bStreamForAccident) {
    ...
    if (!CTheZones::m_CurrLevel || !bStreamForAccident)
        return false;
    int32_t medicModel = ms_aDefaultMedicModel[CTheZones::m_CurrLevel];
    int32_t ambulanceModel = ms_aDefaultAmbulanceModel[CTheZones::m_CurrLevel];
...
void CStreaming::StreamCopModels(int level) {
    ...
    if (FindPlayerWanted(-1) && FindPlayerWanted(-1)->m_nWantedLevel < 3 && level && !m_bDisableCopBikes) {
        const uint32_t timeInMs = CTimer::m_snTimeInMilliseconds;
...
bool CStreaming::StreamFireEngineAndFireman(bool bStreamForFire) {
    ...
    if (!CTheZones::m_CurrLevel || !bStreamForFire)
        return false;
    int32_t firemanModel = ms_aDefaultFiremanModel[CTheZones::m_CurrLevel];
    int32_t fireTruckModel = ms_aDefaultFireEngineModel[CTheZones::m_CurrLevel];
...
void CStreaming::StreamOneNewCar() {
    ...
    if (bCheatActive) {
        int32_t carModelId = CCarCtrl::ChooseCarModelToLoad(carGroupId);
        if (carModelId < 0)
            return;
    ...
    if (m_bBoatsNeeded && (CPopulation::m_LoadedBoats.CountMembers() < 2 ||
        ...
        int32_t carModelId = CCarCtrl::ChooseCarModelToLoad(POPCYCLE_CARGROUP_BOATS);
        if (carModelId >= 0) {
            RequestModel(carModelId, STREAMING_KEEP_IN_MEMORY);
            CPopulation::LoadSpecificDriverModelsForCar(carModelId);
            return;
...
void CStreaming::StreamPedsForInterior(int interiorType) {
    if (interiorType == 0) {
        int32_t numHusbands = CPopulation::GetNumPedsInGroup(POPCYCLE_GROUP_HUSBANDS, 0);
        int32_t randomHusband = rand() % numHusbands;
        int32_t random = CGeneral::GetRandomNumberInRange(3, 9);
        int32_t randomWife = std::max(0, randomHusband - random);
        int32_t numWives = CPopulation::GetNumPedsInGroup(POPCYCLE_GROUP_WIVES, 0);
        ...
        int32_t husbandModelId = CPopulation::GetPedGroupModelId(husbandGroupId, randomHusband);
        ePopcyclePedGroup wifeGroupId = CPopulation::GetPedGroupId(POPCYCLE_GROUP_WIVES, CPopulation::CurrentWorldZone);
        int32_t wifeModelId = CPopulation::GetPedGroupModelId(wifeGroupId, randomWife);
        ClearSlots(2);
        ...
        int32_t numPeds = CPopulation::GetNumPedsInGroup(POPCYCLE_GROUP_SHOPKEEPERS, 0);
        ePopcyclePedGroup groupId = CPopulation::GetPedGroupId(POPCYCLE_GROUP_SHOPKEEPERS, CPopulation::CurrentWorldZone);
        int32_t modelId = CPopulation::GetPedGroupModelId(groupId, rand() % numPeds);
        ClearSlots(1);
        RequestModel(modelId, STREAMING_KEEP_IN_MEMORY);
        ...
        int32_t numPeds = CPopulation::GetNumPedsInGroup(POPCYCLE_GROUP_OFFICE_WORKERS, 0);
        int32_t random = CGeneral::GetRandomNumberInRange(0, numPeds);
        ePopcyclePedGroup groupId = CPopulation::GetPedGroupId(POPCYCLE_GROUP_OFFICE_WORKERS, CPopulation::CurrentWorldZone);
        for (int32_t i = 0; i < TOTAL_LOADED_PEDS; i++) {
            int32_t modelId = CPopulation::GetPedGroupModelId(groupId, (i + random) % numPeds);
            RequestModel(modelId, STREAMING_KEEP_IN_MEMORY);
...
void CStreaming::StreamPedsIntoRandomSlots(int* modelArray) {
    for (int32_t i = 0; i < TOTAL_LOADED_PEDS; i++) {
        int32_t modelId = ms_pedsLoaded[i];
        int32_t pedModelId = modelArray[i];
        if (pedModelId >= 0) {
            if (modelId >= 0) {
                SetModelIsDeletable(modelId);
                SetModelTxdIsDeletable(modelId);
...
void CStreaming::StreamVehiclesAndPeds() {
    ...
    if (CPopCycle::m_NumDealers_Peds > 0.03f) {
        ...
        {
            uint8_t currenZoneFlags = CPopCycle::m_pCurrZoneInfo->Flags2;
    ...
    for (int32_t i = 0; i < CPopulation::GetNumPedsInGroup(POPCYCLE_GROUP_DEALERS, 0); i++) {
        if (i == pedGroupId) {
            RequestModel(CPopulation::GetPedGroupModelId(dealerGroupId, i), STREAMING_GAME_REQUIRED);
        }
        else {
            int32_t modelId = CPopulation::GetPedGroupModelId(dealerGroupId, i);
            SetModelIsDeletable(modelId);
        }
        dealerGroupId = CPopulation::GetPedGroupId(POPCYCLE_GROUP_DEALERS, 0);
    }
    static int32_t framesBeforeStreamingNextNewCar = 0;
...
void CStreaming::StreamVehiclesAndPeds_Always(CVector const& unused) {
    ...
    if (bStream) {
        ...
        if (CPopCycle::m_pCurrZoneInfo) {
            static int32_t lastZonePopulationType = 0;
...
void CStreaming::StreamZoneModels(CVector const& unused) {
    ...
    static int32_t timeBeforeNextLoad = 0;
    if (CPopCycle::m_pCurrZoneInfo->zonePopulationType == ms_currentZoneType) {
        if (timeBeforeNextLoad >= 0) {
            ...
            if (pedId != TOTAL_LOADED_PEDS) {
                int32_t pedModelId = CPopCycle::PickPedMIToStreamInForCurrentZone();
        ...
        int32_t numPedsToLoad = ms_numPedsLoaded;
        for (int32_t i = 0; i < TOTAL_LOADED_PEDS; i++) {
            int32_t modelId = ms_pedsLoaded[i];
            if (modelId >= 0) {
                SetModelIsDeletable(modelId);
        ...
        if (numPedsToLoad < 4)
            numPedsToLoad = 4;
        if (numPedsToLoad > 0) {
            for (int32_t i = 0; i < numPedsToLoad; i++) {
                int32_t pedModelId = CPopCycle::PickPedMIToStreamInForCurrentZone();
                if (pedModelId < 0) {
                    ms_pedsLoaded[i] = -1;
                }
    ...
    static int32_t timeBeforeNextGangLoad = 0;
    if (timeBeforeNextGangLoad >= 0) {
        timeBeforeNextGangLoad--;
    }
    else if (timeBeforeNextGangLoad < 0) {
        timeBeforeNextGangLoad = 550;
        const int32_t currentGangMemberToLoadSlot = CurrentGangMemberToLoad;
        const int32_t nextGangMemberToLoadSlot = CurrentGangMemberToLoad + 1;
        const int32_t nextGangMemberToLoadAnySlot = (CurrentGangMemberToLoad + 1) % 21;
        ...
        for (int32_t groupId = 0; groupId < TOTAL_GANGS; groupId++) {
            ...
            const uint16_t gang = 1 << groupId;
            if (gang & ms_loadedGangs) {
                const int32_t numPedsInGroup = CPopulation::GetNumPedsInGroup(pedGroupId);
                const int32_t currentGangMemberSlot = currentGangMemberToLoadSlot % numPedsInGroup;
                const int32_t nextGangMemberSlot = nextGangMemberToLoadSlot % numPedsInGroup;
                const int32_t nextGangMemberSlot1 = nextGangMemberToLoadAnySlot % numPedsInGroup;
                const int32_t nextGangMemberSlot2 = (nextGangMemberToLoadAnySlot + 1) % numPedsInGroup;
                for (int32_t slot = 0; slot < numPedsInGroup; slot++) {
                    bool bRequestModel = false;
                    if (slot == currentGangMemberSlot || slot == nextGangMemberSlot) {
                        if (slot == nextGangMemberSlot1) {
                            if (slot != currentGangMemberSlot && slot != nextGangMemberSlot) {
                                const int32_t modelId = CPopulation::GetPedGroupModelId(pedGroupId, slot);
...
void CStreaming::StreamZoneModels_Gangs(CVector const& unused) {
    ...
    int32_t gangsNeeded = 0;
    for (int32_t i = 0; i < TOTAL_GANGS; i++) {
    ...
    for (int32_t groupId = 0; groupId < TOTAL_GANGS; groupId++) {
        ...
        if (!(gangsNeeded & gang) || ms_loadedGangs & gang) {
            ...
            int32_t slot1 = CurrentGangMemberToLoad % CPopulation::GetNumPedsInGroup(pedGroupId);
            int32_t modelId1 = CPopulation::GetPedGroupModelId(pedGroupId, slot1);
            int32_t slot2 = (CurrentGangMemberToLoad + 1) % CPopulation::GetNumPedsInGroup(pedGroupId);
            int32_t modelId2 = CPopulation::GetPedGroupModelId(pedGroupId, slot2);
        ...
        if (loadedGangCarGroup.CountMembers() < 1) {
            if (!(gangsNeeded & gang) || ms_loadedGangCars & gang) {
                if (!(gangsNeeded & gang) && (ms_loadedGangCars & gang)) {
                    ...
                    for (int32_t memberId = 0; memberId < loadedCarGroup.CountMembers(); memberId++) {
                        int32_t gangCarModelId = loadedCarGroup.GetMember(memberId);
                        bool bGangHasModelId = false;
                        for (int32_t i = 0; i < CPopulation::m_AppropriateLoadedCars.CountMembers(); ++i) {
                            if (gangCarModelId == CPopulation::m_AppropriateLoadedCars.GetMember(i))
                                bGangHasModelId = true;
                        }
                        for (int32_t index = 0; index < TOTAL_GANGS; index++) {
                            const int32_t carGroupId = index + POPCYCLE_CARGROUP_BALLAS;
                            const uint16_t theGang = (1 << index);
                            if (index != groupId && (theGang & ms_loadedGangs)) {
                                for (int32_t i = 0; i < 23; i++) {
                                    int32_t modelId = CPopulation::m_CarGroups[carGroupId][i];
                ...
                const int32_t carGroupId = groupId + POPCYCLE_CARGROUP_BALLAS;
                const uint16_t numCars = CPopulation::m_nNumCarsInGroup[carGroupId];
                const int32_t modelId = CPopulation::m_CarGroups[carGroupId][rand() % numCars];
                if (ms_aInfoForModel[modelId].m_nLoadState != LOADSTATE_LOADED)
                    RequestModel(modelId, STREAMING_KEEP_IN_MEMORY);
            }
        }
