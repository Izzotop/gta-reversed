...
uint32_t& gStreamFileCreateFlags = *(uint32_t*)0x8E3FE0;
CdStream*& gCdStreams = *(CdStream**)0x8E3FFC;
int32_t& gStreamCount = *(int32_t*)0x8E4090;
int32_t& gOpenStreamCount = *(int32_t*)0x8E4094;
int32_t& gStreamingInitialized = *(int32_t*)0x8E3FE4;
int32_t& gOverlappedIO = *(int32_t*)0x8E3FE8;
...
uint32_t& gLastCdStreamPosn = *(uint32_t*)0x8E4898;
...
bool __cdecl CdStreamRead(int32_t streamId, uint8_t* lpBuffer, uint32_t offsetAndHandle, int32_t sectorCount)
{
    ...
    const uint32_t sectorOffset = offsetAndHandle & 0xFFFFFF;
...
[[noreturn]] DWORD WINAPI CdStreamThread(LPVOID lpParam)
{
    while (true) {
        ...
        const int32_t streamId = GetFirstInQueue(&gStreamQueue);
        CdStream& stream = gCdStreams[streamId];
        stream.bInUse = true;
        if (stream.status == eCdStreamStatus::READING_SUCCESS) {
...
{
    SetLastError(NO_ERROR);
    for (int32_t i = 0; i < gStreamCount; i++) {
...
{
    for (int32_t i = 0; i < MAX_CD_STREAM_HANDLES; i++) {
    ...
    uint8_t* pAllocatedMemory = CMemoryMgr::MallocAlign(STREAMING_SECTOR_SIZE, bytesPerSector);
    SetLastError(NO_ERROR);
    gOpenStreamCount = 0;
...
void __cdecl CdStreamRemoveImages()
{
    ...
    for (int32_t i = 0; i < gOpenStreamCount; i++) {
        SetLastError(NO_ERROR);
