...
int32_t& CVisibilityPlugins::ms_atomicPluginOffset = *(int32_t*)0x8D608C;
int32_t& CVisibilityPlugins::ms_clumpPluginOffset = *(int32_t*)0x8D6094;
int32_t& CVisibilityPlugins::ms_framePluginOffset = *(int32_t*)0x8D6090;
...
int32_t CVisibilityPlugins::CalculateFadingAtomicAlpha(CBaseModelInfo* modelInfo, CEntity* entity, float distance) {
    ...
    return static_cast<int32_t>(modelInfo->m_nAlpha * fFade);
}

void CVisibilityPlugins::ClearAtomicFlag(RpAtomic* pRpAtomic, uint16_t flag) {
    ATOMICPLG(pRpAtomic, m_flags) &= ~flag;
}

RpAtomic* CVisibilityPlugins::ClearAtomicFlagCB(RpAtomic* atomic, void* data) {
    uint16_t flag = *reinterpret_cast<uint16_t*>(&data);
    ATOMICPLG(atomic, m_flags) &= ~flag;
    return atomic;
}

void CVisibilityPlugins::ClearClumpForAllAtomicsFlag(RpClump* pRpClump, uint16_t flag) {
...
float CVisibilityPlugins::GetDotProductWithCameraVector(RwMatrixTag* atomicMatrix, RwMatrixTag* clumpMatrix, uint16_t flags) {
...
int32_t CVisibilityPlugins::GetFrameHierarchyId(RwFrame* pRwFrame) {
    return FRAMEPLG(pRwFrame, m_hierarchyId);
}

int16_t CVisibilityPlugins::GetModelInfoIndex(RpAtomic* pRpAtomic) {
    return ATOMICPLG(pRpAtomic, m_modelId);
}

int16_t CVisibilityPlugins::GetUserValue(RpAtomic* pRpAtomic) {
...
void CVisibilityPlugins::RenderAlphaAtomic(RpAtomic* atomic, int32_t alpha) {
    uint8_t alphas[152];
    RpGeometry* geometry = atomic->geometry;
    uint32_t geometryFlags = RpGeometryGetFlags(geometry);
    RpGeometrySetFlags(geometry, geometryFlags | rpGEOMETRYMODULATEMATERIALCOLOR);
    const int32_t numMaterials = RpGeometryGetNumMaterials(geometry);
    for (int32_t i = 0; i < numMaterials; i++) {
        ...
        color->alpha = std::min(color->alpha, (uint8_t)alpha);
    }
...
RpAtomic* CVisibilityPlugins::RenderAtomicWithAlphaCB(RpAtomic* pRpAtomic, void* pData) {
    if (RpAtomicGetFlags(pRpAtomic) & rpATOMICRENDER)
        RenderAlphaAtomic(pRpAtomic, *reinterpret_cast<int32_t*>(pData));
    return pRpAtomic;
}
...
void CVisibilityPlugins::RenderEntity(CEntity* entity, int unused, float distance) {
    ...
    if (!entity->m_bDistanceFade) {
        ...
        RwRenderStateSet(rwRENDERSTATEALPHATESTFUNCTIONREF, (void*)0);
        int32_t alpha = CalculateFadingAtomicAlpha(pModelInfo, entity, distance);
...
void CVisibilityPlugins::RenderFadingAtomic(CBaseModelInfo* modelInfo, RpAtomic* atomic, int32_t alpha) {
...
void CVisibilityPlugins::RenderFadingClump(CBaseModelInfo* modelInfo, RpClump* clump, int32_t alpha) {
    if (modelInfo->bAdditiveRender)
        RwRenderStateSet(rwRENDERSTATEDESTBLEND, (void*)rwBLENDONE);
...
RpAtomic* CVisibilityPlugins::RenderPedCB(RpAtomic* pRpAtomic) {
    ...
    if (distanceSquared >= ms_pedLodDist)
        return pRpAtomic;
    int32_t alpha = GetClumpAlpha(RpAtomicGetClump(pRpAtomic));
    if (alpha == 255) {
        AtomicDefaultRenderCallBack(pRpAtomic);
        return pRpAtomic;
    }
...
RpAtomic* CVisibilityPlugins::RenderTrainHiDetailAlphaCB(RpAtomic* pRpAtomic) {
    ...
    RwMatrixTag* atomicMatrix = RwFrameGetLTM(RpAtomicGetFrame(pRpAtomic));
    uint16_t atomicFlags = GetAtomicId(pRpAtomic);
    RwMatrixTag* clumpMatrix = RwFrameGetLTM(RpClumpGetFrame(RpAtomicGetClump(pRpAtomic)));
...
void CVisibilityPlugins::RenderWeaponPedsForPC() {
    ...
    for (; link != &ms_weaponPedsForPC.usedListHead; link = link->prev) {
        ...
        if (ped && ped->m_pWeaponObject) {
            ...
            const int32_t boneID = activeWeapon.m_nType != WEAPON_PARACHUTE ? BONE_R_HAND : BONE_SPINE1;
            int32_t animIDIndex = RpHAnimIDGetIndex(pRpAnimHierarchy, boneID);
            ...
            int8_t weaponSkill = ped->GetWeaponSkill();
            if (CWeaponInfo::GetWeaponInfo(activeWeapon.m_nType, weaponSkill)->flags.bTwinPistol) {
                int32_t animIDIndex = RpHAnimIDGetIndex(pRpAnimHierarchy, BONE_L_HAND);
...
void CVisibilityPlugins::SetAtomicFlag(RpAtomic* pRpAtomic, uint16_t flag) {
    ATOMICPLG(pRpAtomic, m_flags) |= flag;
}

void CVisibilityPlugins::SetClumpForAllAtomicsFlag(RpClump* pRpClump, uint16_t flag) {
    RpClumpForAllAtomics(pRpClump, SetAtomicFlagCB, (void*)flag);
}

void CVisibilityPlugins::SetAtomicId(void* pRpAtomic, int16_t id) {
...
void CVisibilityPlugins::SetFrameHierarchyId(RwFrame* pRwFrame, int32_t id) {
