...
int32_t MAX_INVISIBLE_ENTITY_PTRS = 150;
int32_t MAX_VISIBLE_ENTITY_PTRS = 1000;
int32_t MAX_VISIBLE_LOD_PTRS = 1000;
int32_t MAX_VISIBLE_SUPERLOD_PTRS = 50;
...
{
    plugin::Call<0x72CAE0,CVector2D*, int32_t, tScanFunction>(points,pointsCount, scanFunction);
}
...
void CRenderer::ProcessLodRenderLists() {
    ...
    const uint8_t displaySuperLowLodFlag = 0x80u; // yes, this is very hacky. Blame R*
...
void CRenderer::RenderRoads() {
    ...
    DeActivateDirectional();
    SetAmbientColours();
    for (int32_t i = 0; i < CRenderer::ms_nNoOfVisibleEntities; ++i) {
    ...
    if (!pBaseAtomicModelInfo)
    {
        if (pBaseModelInfo->GetModelType() != MODEL_INFO_CLUMP && pBaseModelInfo->GetModelType() != MODEL_INFO_WEAPON)
        {
            if (FindPlayerVehicle(-1, false) == pEntity && gbFirstPersonRunThisFrame && CReplay::Mode != REPLAY_MODE_1) {
                uint32_t dwDirectionWasLooking = TheCamera.m_aCams[TheCamera.m_nActiveCam].m_nDirectionWasLooking;
...
void CRenderer::ScanSectorList_ListModels(int32_t sectorX, int32_t sectorY) {
    ...
    for (int32_t scanListIndex = 0; scanListIndex < TOTAL_ENTITY_SCAN_LISTS; scanListIndex++) {
    ...
    if (sectorX >= 0 && sectorY >= 0 && sectorX < MAX_LOD_PTR_LISTS_X && sectorY < MAX_LOD_PTR_LISTS_Y) {
        ...
        for (CPtrNode* pNode = list.GetNode(); pNode; pNode = pNode->pNext) {
            ...
            if (entity->m_nScanCode != CWorld::ms_nCurrentScanCode) {
                entity->m_nScanCode = CWorld::ms_nCurrentScanCode;
                float fDistance = 0.0f;
                int32_t visibility = CRenderer::SetupBigBuildingVisibility(entity, &fDistance);
...
// 0x555CB0
void CRenderer::RequestObjectsInDirection(CVector const& posn, float angle, int32_t modelRequestFlags) {
    RwMatrix matrix;
