...
int32_t CWorld::TOTAL_PLAYERS = 2;
...
CPtrListSingleLink& CWorld::GetLodPtrList(int32_t x, int32_t y) {
    x = clamp<int32_t>(x, 0, MAX_LOD_PTR_LISTS_X - 1);
    y = clamp<int32_t>(y, 0, MAX_LOD_PTR_LISTS_Y - 1);
    return ms_aLodPtrLists[y][x];
    //return plugin::CallAndReturn<CPtrListSingleLink*, 0x4072C0, int32_t, int32_t>(x, y);
}
