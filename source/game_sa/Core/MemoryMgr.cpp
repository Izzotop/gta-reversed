...
uint8_t *CMemoryMgr::MallocAlign(size_t size, uintptr_t align)
{
#if defined(USE_GTASA_ALLOCATOR) || defined(USE_DEFAULT_FUNCTIONS)
    return plugin::CallAndReturn<uint8_t*, 0x72F4C0, size_t, uintptr_t>(size, align);
#else
    uint8_t *ptr = (uint8_t *) malloc(size + align)
    if (ptr == nullptr)
        return nullptr;

    uintptr_t *ptr2 = (uintptr_t *) ((uintptr_t) ptr + align & ~(align - 1));
