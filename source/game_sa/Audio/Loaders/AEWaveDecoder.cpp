    ...
    if (readed == HEADER_READ_SIZE && bitsPerSample == 16 && (channelCount == 1 || channelCount == 2))
    {
        ...
        while (true)
        {
            if (dataStream->FillBuffer(tag, sizeof(tag)) != sizeof(tag))
                // EOF
                return false;
            if (dataStream->FillBuffer(&size, sizeof(uint32_t)) != sizeof(uint32_t))
                return false;
...
size_t CAEWaveDecoder::FillBuffer(void* dest, size_t size)
{
    ...
    switch (channelCount)
    {
        case 1:
        {
            ...
            int16_t *byteStream = reinterpret_cast<int16_t *> (dest);

            // Duplicate channel data
            for (int i = static_cast<int> (readed / sizeof(int16_t)) - 1; i >= 0; i--)
...
long CAEWaveDecoder::GetStreamLengthMs()
{
    ...
    if (initialized)
    {
        uint32_t samples = dataChunkSize / blockAlign;
...
long CAEWaveDecoder::GetStreamPlayTimeMs()
{
    ...
    if (initialized)
    {
        uint32_t posByte = dataStream->GetCurrentPosition() - dataPosition;
        uint32_t posSamples = posByte / blockAlign;
        // More precise way would be involving using GCD
...
void CAEWaveDecoder::SetCursor(unsigned long pos)
{
    ...
    if (initialized)
    {
        ...
        uint32_t posByte = pos * sampleRate / 1000 * blockAlign;
